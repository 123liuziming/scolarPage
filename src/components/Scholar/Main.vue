...
import { gql_sendMessage, gql_getUserInfo } from "@/graphql/social";
...
export default {
  ...
  methods: {
    async addUserToParticipant(userId) {
      try {
        const response = await gql_getUserInfo(userId);
        this.$emit("messageScholar", response.data.getUserInfo);
      } catch (err) {
        console.log(err);
      }
    },
